...
def beam_error(
    ...
    **kwargs,
    ...
    """compute fom solution and rom error"""

    logger = getLogger("src.beam.error", level=loglvl, filename=logfile)
    timer = Timer("error")
    ...
    beam_problem.setup_fine_grid()
    beam_problem.setup_coarse_space()
    beam_problem.setup_fine_space()

    coarse_grid = beam_problem.coarse_grid
    fine_grid = beam_problem.fine_grid
    ...
    V = beam_problem.V  # fine space
    fe_family = W.element.basix_element.family

    # Dirichlet data
    fom_dirichlet_bc = beam_problem.get_dirichlet()["homogeneous"]

    # Neumann data
    fom_neumann_bc = beam_problem.get_neumann()["fom"]
    ...
    fom_problem.A.destroy()
    fom_problem.b.destroy()
    del fine_grid
    ...
    for cell_index in range(coarse_grid.num_cells):
        ...
        if product_name == "energy":
            Ω = Domain(subdomain, cell_markers=ct)
            problem = LinearElasticityProblem(
                Ω, V_i, E=E, NU=NU, plane_stress=plane_stress
            )
            form = problem.form_lhs
            inner_product = InnerProduct(V_i, form, bcs=())
        else:
            inner_product = InnerProduct(V_i, product_name, bcs=())

        matrix = inner_product.assemble_matrix()
        product_op = FenicsxMatrixOperator(matrix, V_i, V_i)
        ...
        for modes_it, num_modes in enumerate(list(range(NUM_MAX_MODES + 1))):
            ...
            rom = source.make_array([u_rom_local.vector])
            err = fom - rom
            fom_norm = fom.norm(product_op)
            err_norm = err.norm(product_op)

            loc_err_norm.append(err_norm)
            loc_fom_norm.append(fom_norm)
            logger.debug(f"{num_modes=}     {err_norm=}")

        loc_err_norm = np.hstack(loc_err_norm)
        loc_fom_norm = np.hstack(loc_fom_norm)
        LOC_ERR.append(loc_err_norm)
        LOC_FOM.append(loc_fom_norm)

    LOC_ERR = np.array(LOC_ERR)
    LOC_FOM = np.array(LOC_FOM)

    # column i contains local err norm for num_modes=i
    GL_ERR = np.sqrt(np.sum(LOC_ERR ** 2, axis=0))
    GL_FOM = np.sqrt(np.sum(LOC_FOM ** 2, axis=0))
    GL_REL_ERR = GL_ERR / GL_FOM

    outstream = example_config.error(distribution, real)
    NUM_DOFS = []
    for array in U_ROM:
        NUM_DOFS.append(array.size)
    np.savez(outstream.as_posix(), err=GL_REL_ERR, abserr=GL_ERR, num_modes=NUM_MODES, num_dofs=NUM_DOFS)
