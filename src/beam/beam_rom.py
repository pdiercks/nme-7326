...
def beam_rom(
    ...
    **kwargs,
    ...
    """compute rom solution for beam example"""

    logger = getLogger("src.beam.rom", level=loglvl, filename=logfile)
    timer = Timer("rom")
    ...
    left_boundary_vertices = coarse_grid.locate_entities_boundary(
        0, on_left_boundary
    )
    ...
    for num_modes in range(NUM_MAX_MODES + 1):
        ...
        for cell_index in range(coarse_grid.num_cells):
            ...
            local_basis = select_modes(
                    bases[cell_index], max_modes[cell_index], dofs_per_edge[cell_index]
                    )
        ...
        for cell_index in range(coarse_grid.num_cells):
            ...
            # in case of empirical basis no action required
            # for fine scale modes on Σ_D or Σ_N
            if example_config.name.startswith("hbeam"):
                # add constraints for fine scale x-component on left boundary
                left_boundary_facets = coarse_grid.locate_entities_boundary(
                        1, on_left_boundary
                        )
                for ent in left_boundary_facets:
                    dofs = dofmap.entity_dofs(1, ent)
                    for d in dofs:
                        if not d % 2:
                            rom_bcs.update({d: 0.0})
